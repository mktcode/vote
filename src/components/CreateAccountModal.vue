...
function createAccount() {
  userManager.create(alias.value, password.value, (ack: any) => {
    if (ack.err) {
      error.value = ack.err;
      return;
    }
...
<template>
  <ModalBase :is-open="isOpen" @close="$emit('close')">
    <template #title> Create a an account </template>
    <template #content>
      ...
      <div class="mt-5 flex space-x-2">
        ...
        <button class="grow" @click="createAccount">Create account</button>
      </div>
    </template>
  </ModalBase>
</template>
